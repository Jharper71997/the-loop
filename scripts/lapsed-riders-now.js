// Pull lapsed riders: rode before but not in the last 8 weeks from today (2026-07-29).
// node scripts/lapsed-riders-now.js
...
// 8 weeks before 2026-07-29
const CUTOFF_DATE = new Date('2026-06-03')
...
async function main() {
  ...
  for (const r of riders) {
    ...
    // Lapsed = last ride before the cutoff (more than 8 weeks ago)
  ...
  // Sort: repeat riders with phone first, then by ride count desc
  lapsed.sort((a, b) => {
  ...
  lapsed.slice(0, 30).forEach(r => {
    console.log(` [${r.isRepeat?'REPEAT':'once '}] ${r.name.padEnd(25)} rides:${r.rideCount} last:${r.lastRide} phone:${r.phone||'none'} email:${r.email||'none'}`)
  })

  const out = 'C:/Users/jacob/OneDrive/Desktop/lapsed-riders-2026-07-29.json'
