...
async function upsertGroupForTicket(supabase, ticket, order) {
  ...
  if (existing) {
    if (!existing.schedule && start.time) {
      const schedule = buildDefaultSchedule(start.time, firstStopName(ticket))
      if (schedule) {
        await supabase.from('groups').update({ schedule }).eq('id', existing.id)
      }
    }
    return existing
  }

  const schedule = start.time ? buildDefaultSchedule(start.time, firstStopName(ticket)) : null
