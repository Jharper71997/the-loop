...
import { textUnsignedContact } from './waiver'
...
export async function handleOrder(supabase, order, opts = {}) {
  const { skipWaiverSms = false } = opts
  ...
  for (const ticket of tickets) {
    ...
    // Send waiver-reminder SMS to the TT rider. TT buyers never pass through
    // the /book inline waiver, so without this they arrive unsigned with no
    // prompt. textUnsignedContact dedupes on a 24h window so multi-ticket
    // orders don't spam. Backfills skip this — the 24h window would mostly
    // dedupe anyway and the SMS call is the slowest part of handleOrder.
    if (!skipWaiverSms) {
      try {
        const { data: fresh } = await supabase
          ...
          .eq('id', contact.id)
          .maybeSingle()
        if (fresh) await textUnsignedContact(supabase, fresh)
      } catch (err) {
        console.error('[ticketTailor] waiver SMS failed', err)
      }
    }
