...
function matchStopIndex(ticket, schedule) {
  ...
  const raw = ticket?.description || ticket?.name || ''
  if (!raw) return null

  // Preferred: ticket name in "Bar Name - 7:30 PM" format → use parsed.name.
  const parsed = parseTicketTypeName(raw)
  const candidates = []
  if (parsed?.name) candidates.push(parsed.name)

  // Fallback: TT ticket types are often just "HideAway Lounge" with no time
  // suffix, so the parser fails. Try matching the raw description directly,
  // and the substring before " - " (which strips trailing notes like
  // " - Free Ticket" or " - VIP").
  candidates.push(raw)
...
async function upsertOrderItemsFromTickets(supabase, { orderId, enrichedTickets, buyer }) {
  // TT issued_tickets almost always ship with first_name/last_name/phone null —
  // only buyer_details carries data. Fall back to buyer info so the admin
  // Roster shows the actual buyer for every seat instead of "(unnamed)".
  const buyerFirst = buyer?.firstName || null
  const buyerLast = buyer?.lastName || null
  ...
  for (const { ticket, contact, stopIdx } of enrichedTickets) {
    ...
    rows.push({
      ...
      rider_first_name: ticket.first_name || buyerFirst,
      rider_last_name: ticket.last_name || buyerLast,
      rider_email: normalizeEmail(ticket.email) || buyerEmail,
      rider_phone: normalizePhone(ticket.phone) || buyerPhone,
      unit_price_cents: ticketPriceCents(ticket),
      stop_index: stopIdx,
      tt_ticket_id: String(ticket.id),
      tt_barcode: ticket.barcode ? String(ticket.barcode) : null,
    })
  }
...
async function upsertGroupForTicket(supabase, ticket, order) {
  ...
  // Fallback: a native Loop already exists for this date with no tt_event_id.
  // Attach the incoming TT order to it instead of spawning a duplicate orphan
  // group. This is the post-Singenuity migration case — admins create Loops
  // natively in /admin/groups/new and a straggler TT sale arrives later.
  if (!existing && eventDate) {
    ...
    if (nativeMatch) {
      const { error: linkErr } = await supabase
        .from('groups')
        .update({ tt_event_id: eventId })
        .eq('id', nativeMatch.id)
      if (linkErr) {
        console.error('[ticketTailor] native group tt_event_id link failed', linkErr)
      } else {
        existing = nativeMatch
      }
    }
  }
...
async function upsertContact(supabase, r) {
  // Need at least one identifier to dedupe by — without phone OR email there's
  // no way to merge repeat purchases, and the row would be unreachable anyway.
  // Names alone aren't unique enough. Phone is preferred, email is the
  // fallback (TT events sometimes ship buyer_details with email but no phone).
  if (!r.phone && !r.email) return null

  // Sparse-merge patch — only overwrite columns when the new payload has a
  // value. TT sometimes ships orders where buyer_details.phone or name is
  // missing, and a blanket update would blank an already-populated contact.
  function patchFor() {
    const p = { updated_at: new Date().toISOString() }
    if (r.firstName) p.first_name = r.firstName
    if (r.lastName) p.last_name = r.lastName
    if (r.email) p.email = r.email
    if (r.phone) p.phone = r.phone
    if (r.ticketType) p.ticket_type = r.ticketType
    if (r.orderId) p.last_tt_order_id = r.orderId
    if (r.ticketId) p.last_ticket_id = r.ticketId
    if (typeof r.smsConsent === 'boolean') p.sms_consent = r.smsConsent
    return p
  }
  ...
  async function findExisting() {
    ...
    if (r.email) {
      // Use eq() not ilike() — PostgREST treats '*' as a wildcard, so an
      // ilike on a value containing '*' (TT once shipped buyer_email='****')
      // matches every row and returns a random one via limit(1), then the
      // subsequent update collides with the real holder of that email.
      // normalizeEmail() already lowercases both sides, so eq is correct.
      const { data } = await supabase
        .from('contacts')
        .select('id')
        .eq('email', r.email)
  ...
  if (existing) {
    const upd = await supabase
      ...
      .update(patchFor())
      .eq('id', existing.id)
    if (!upd.error) return existing

    // Update collided on a unique constraint — usually a legacy split:
    // existing has phone, but a different row already owns the email we're
    // trying to set (or vice versa). Don't fail the order; retry the patch
    // without the conflicting field so we still update name/timestamp/etc.
    ...
    if (isDup) {
      const safe = patchFor()
      if (/email/i.test(msg)) delete safe.email
      if (/phone/i.test(msg)) delete safe.phone
      const retry = await supabase
        .from('contacts')
        .update(safe)
        .eq('id', existing.id)
      if (!retry.error) return existing
      throw new Error(`contact update retry failed: ${retry.error.message || retry.error.code}`)
    }
    throw new Error(`contact update failed: ${upd.error.message || upd.error.code}`)
  }
  ...
  // Self-heal: if findExisting missed (PostgREST/Supabase JS quirk we've seen
  // intermittently), the unique-constraint error tells us the row IS there.
  // Re-query by phone or email and update instead of bailing the whole order.
  ...
  if (isDuplicate) {
    ...
    if (recovered) {
      const { error: updErr } = await supabase
        ...
        .update(patchFor())
        .eq('id', recovered.id)
      if (updErr) throw new Error(`contact recovery update failed: ${updErr.message || updErr.code}`)
      return recovered
    }
  }
  throw new Error(`contact insert failed: ${error.message || error.code}`)
}
...
function normalizeEmail(raw) {
  ...
  // Reject obvious junk values TT has actually shipped: literal "****" (when a
  // buyer entered asterisks to skip a required field), bare strings with no
  // '@', and PostgREST wildcards that would poison ilike queries downstream.
