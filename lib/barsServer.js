...
import { getBarByName as getStaticBarByName, getSurfBarByName, getMarinesBarByName } from './bars'

// Pick the right static directory for a business: Brew uses BARS, Surf City uses
// SURF_BARS, The Loop (Marines) uses MARINES_BARS. Keeps a stop named like a bar
// in another business from matching the wrong directory.
function staticLookup(business, name) {
  if (business === 'surf') return getSurfBarByName(name)
  if (business === 'marines') return getMarinesBarByName(name)
  return getStaticBarByName(name)
}
...
export async function lookupBarByName(supabase, name, { business = 'brew' } = {}) {
  if (!name) return null

  const stat = staticLookup(business, name)
...
export async function lookupBarsByNames(supabase, names, { business = 'brew' } = {}) {
  const out = new Map()
  ...
  for (const n of names) {
    const stat = staticLookup(business, n)
    if (stat && Number.isFinite(stat.lat) && Number.isFinite(stat.lng)) {
      out.set(n, stat)
    } else {
      unresolved.push(n)
...
export async function resolveScheduleStops(supabase, schedule, { business = 'brew' } = {}) {
  ...
  const barLookup = needLookup.length
    ? await lookupBarsByNames(supabase, needLookup, { business }).catch(() => new Map())
    : new Map()
