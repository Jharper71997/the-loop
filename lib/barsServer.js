...
import { getBarByName as getStaticBarByName, getSurfBarByName } from './bars'

// Pick the right static directory for a business: Brew uses BARS, Surf City
// uses SURF_BARS. Keeps a Surf stop named like a Brew bar from matching the
// wrong directory.
function staticLookup(business, name) {
  return business === 'surf' ? getSurfBarByName(name) : getStaticBarByName(name)
}
