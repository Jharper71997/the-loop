...
// them to riders.
//
// on_sale alone is not enough, though: a private party HAS to be on_sale to be
// bookable at all, which is why every charter we sold used to sit on this page
// next to the Friday loops until its date passed. is_private is the second
// half of that question — bookable and advertised are not the same thing.
//
...
export async function getUpcomingLoops({ limit = 12, business = 'brew' } = {}) {
  let sb
  try {
    sb = supabaseAdmin()
  } catch (err) {
  ...
  try {
    eventsRes = await sb
      ...
      .eq('status', 'on_sale')
      .eq('kind', business)   // scope to the active business; never cross-surface another loop's events
      .eq('is_private', false) // private parties are bookable but never advertised — see lib/parties.js
  ...
  if (allNames.size) {
    try {
      barLookup = await lookupBarsByNames(sb, [...allNames], { business })
    } catch (err) {
...
export async function getActiveAdminLoop(business = 'brew') {
  let sb
  ...
  const { data: groups, error } = await sb
    ...
    .eq('kind', business)   // scope to the active business (Brew/Surf); Marines has its own active-loop loader
