...
// First letter of last name, lowercased, alphanumeric only. Returns '' if
// last name has no usable letters.
export function lastInitial(lastName) {
...
// Public-facing display name: "Brittany S." (first + last initial + period).
// Falls back to bare first name if last name is missing.
...
// Look a bartender up by email and/or phone.
//
// This used to be one `.or()` call built by string concatenation:
//   .or(`email.ilike.${email},phone.eq.${phone}`)
// supabase-js passes an `.or()` string to PostgREST verbatim — it escapes
// nothing — so a comma inside the interpolated value injected an extra filter.
// On a public service-role route that was an auth bypass and a table-walking
// oracle. Passing the value as its own argument (`.ilike(col, val)`) URL-encodes
// it, so it can only ever be compared, never parsed as filter syntax.
//
// Note `.ilike` still treats `_` as a single-character wildcard. normalizeEmail
// rejects `%`, and a lone `_` can at most match a near-identical address, which
// the two-row cap below already handles. It cannot add a condition.
export async function findBartendersByContact(supabase, { email, phone }, cols) {
  const select = cols || 'slug, display_name, bar, qr_image_url, active, share_code, email, phone'
  const seen = new Map()
  let lastError = null

  for (const [column, value, op] of [['email', email, 'ilike'], ['phone', phone, 'eq']]) {
    if (!value) continue
    const q = supabase.from('bartenders').select(select).limit(2)
    const { data, error } = await (op === 'ilike' ? q.ilike(column, value) : q.eq(column, value))
    if (error) { lastError = error; continue }
    for (const row of data || []) if (!seen.has(row.slug)) seen.set(row.slug, row)
  }

  // Only surface an error if nothing matched — one leg failing while the other
  // finds the row shouldn't fail the lookup.
  if (!seen.size && lastError) return { data: null, error: lastError }
  return { data: [...seen.values()].slice(0, 2), error: null }
}
