...
// map to attach lat/lng to a schedule entry. Falls back through three
// progressively-fuzzier matches so admin schedule typos don't drop pins:
//   1. Exact lowercase
...
export function getBarByName(name) {
  ...
  if (!norm) return null
  const normExact = BARS.find(b => normalizeBarName(b.name) === norm)
