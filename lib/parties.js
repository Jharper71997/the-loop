...
//   groups   1 row  — holds the ROUTE (schedule jsonb). Stop 0 is the pickup,
//                     which can be anywhere they want.
//   events   1 row  — is_private = true, access_token set, status 'on_sale'
//                     (a party must be on_sale to be bookable; is_private is
//                     what stops it being advertised), plus party_pricing.
//   ticket_types    — depends on how the group is splitting the money:
//
//     flat        2 fares. A paid "Whole shuttle (organizer)" seat carrying the
//                 entire quote, and a $0 "Guest seat" per friend. One purchase,
//                 so the money is guaranteed by whoever organised it. Stripe
//                 accepts $0 line items as long as the ORDER total is above
//                 zero, which it always is because the organizer's seat is on
//                 the same order.
//
//     per_person  1 fare, "Seat", at the per-head price. The link behaves
//                 exactly like a public loop: riders buy their own seat, or one
//                 person buys several and either signs for them or forwards
//                 each a claim link. This fare MUST carry an explicit capacity
//                 — see the note on it in the builder.

import { randomBytes } from 'crypto'

// Fare names are rider-facing — they show up on the booking page and on the
// boarding pass.
//
// FLAT: the organizer carries the whole price and everyone else rides on a $0
// seat, so the money is guaranteed by one purchase.
export const ORGANIZER_FARE = 'Whole shuttle (organizer)'
export const GUEST_FARE = 'Guest seat'
// PER PERSON: one priced seat, bought as many times as the group needs —
// individually, or several at once by whoever is organising.
export const SEAT_FARE = 'Seat'

export const PRICING_FLAT = 'flat'
export const PRICING_PER_PERSON = 'per_person'

// Defaults to flat so a party built before migration 050 still reads correctly.
export function partyPricing(event) {
  return event?.party_pricing === PRICING_PER_PERSON ? PRICING_PER_PERSON : PRICING_FLAT
}

export function isPerPerson(event) {
  return partyPricing(event) === PRICING_PER_PERSON
}
...
export async function getPartyByToken(sb, token) {
  ...
  const { data: event, error } = await sb
    .from('events')
    .select('id, name, event_date, pickup_time, description, status, capacity, cover_image_url, group_id, kind, is_private, access_token, party_pricing')
...
// The headline number for a party.
//
// Flat: the organizer seat carries the whole quote. Per person: the one priced
// seat IS the number. Both fall back to the dearest fare so a party built by
// hand with differently-named tickets still shows a price instead of "$0".
...
// What the price means, in the words the customer needs to read next to it.
// Getting this backwards is the difference between "$20 covers your group" and
// "$20 each", so it is derived from the event, never guessed from the number.
export function priceCaption(event) {
  return isPerPerson(event) ? 'per person' : 'the whole shuttle, not per person'
}
