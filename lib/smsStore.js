...
export async function upsertProviderMessagesBatch(sb, msgs, opts = {}) {
  ...
  // select above. Its row wins; the next run fills any gap. Chunked because a
  // single 500-row request dies with "fetch failed" before PostgREST answers.
  for (const batch of chunks(inserts, 100)) {
    const { data, error } = await sb
      .from('sms_messages')
      .upsert(batch, { onConflict: 'provider_message_id', ignoreDuplicates: true })
      .select('id')
