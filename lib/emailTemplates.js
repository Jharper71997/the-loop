...
export function bookingConfirmationHtml({ buyer, event, ticketLinks = [], waiverLink, hasSignedWaiver, claimLinks = [] }) {
  ...
  const ticketsBlock = ticketLinks.length
    ? `<p style="margin:0 0 14px;color:${INK};font-size:14px;">Show this QR to the driver when you board:</p>
       ${ticketLinks.map(t => {
         const qrSrc = `${appUrl()}/api/qr-image?size=480&data=${encodeURIComponent(t.url)}`
         return `
         <div style="margin:0 0 18px;padding:16px;background:#ffffff;border-radius:14px;text-align:center;">
           ${t.name ? `<p style="margin:0 0 10px;color:#0a0a0b;font-size:13px;font-weight:700;letter-spacing:0.04em;text-transform:uppercase;">${escape(t.name)}</p>` : ''}
           <img src="${escape(qrSrc)}" alt="Ticket QR code" width="240" height="240" style="display:block;margin:0 auto;width:240px;height:240px;border:0;" />
