  ...
  const includedBlock = `
    <div style="margin:24px 0;padding:16px;background:#15151a;border:1px solid #2a2a31;border-radius:10px;">
      ...
      <ul style="margin:0;padding:0 0 0 18px;color:${INK};font-size:14px;line-height:1.6;">
        <li>One seat on the Brew Loop shuttle for the night</li>
  ...
  const appBlock = `
    <div style="margin:24px 0;padding:16px;background:#15151a;border:1px solid #2a2a31;border-radius:10px;">
      ...
      <p style="margin:0 0 8px;color:${INK};font-size:14px;line-height:1.5;">
        <strong>Live tracking:</strong> <a href="${appUrl()}/track" style="color:${GOLD};text-decoration:none;font-weight:600;">${appUrl().replace(/^https?:\/\//, '')}/track</a> shows where the shuttle is, what bar is next, and how long until your stop.
      </p>
      <p style="margin:0 0 8px;color:${INK};font-size:14px;line-height:1.5;">
        <strong>Your tickets:</strong> <a href="${appUrl()}/my-tickets" style="color:${GOLD};text-decoration:none;font-weight:600;">${appUrl().replace(/^https?:\/\//, '')}/my-tickets</a> — enter your phone to pull up the QR if you lose this email.
      </p>
...
export function bookingConfirmationText({ buyer, event, ticketLinks = [], waiverLink, hasSignedWaiver, claimLinks = [] }) {
  ...
  body += '- One seat on the Brew Loop shuttle for the night\n'
  ...
  body += 'USE THE APP\n'
  body += `- Live tracking: ${appUrl()}/track\n`
  body += `- Your tickets: ${appUrl()}/my-tickets (enter your phone)\n`
...
<html><body style="margin:0;padding:0;background:#0a0a0b;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;color:${INK};">
  <div style="max-width:540px;margin:0 auto;padding:24px 20px;">
    <div style="text-align:center;margin-bottom:18px;">
      <p style="margin:0;color:${GOLD};font-size:11px;letter-spacing:0.22em;text-transform:uppercase;font-weight:700;">Jville Brew Loop · 1 hour out</p>
    </div>
    ...
    <div style="margin:18px 0;padding:14px;background:#15151a;border:1px solid #2a2a31;border-radius:10px;">
      ...
      <p style="margin:0;color:${INK};font-size:14px;line-height:1.5;">
        <a href="${appUrl()}/track" style="color:${GOLD};text-decoration:none;font-weight:600;">${appUrl().replace(/^https?:\/\//, '')}/track</a> — see where the bus is, what bar is next, and your live arrival.
      </p>
    </div>
...
export function ticketReminderText({ rider, event, ticketUrl, stopName, stopTime }) {
  ...
  body += `Track the shuttle: ${appUrl()}/track\n\n`
  body += `Jville Brew Loop`
  return body
}
