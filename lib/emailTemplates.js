...
function footerHtml(kind) {
  ...
  // Brew: sender identity, postal address once we have one (CAN-SPAM wants it
  // on anything promotional), and the policy links. These emails are about a
  // booking, so there is no marketing list to unsubscribe from. If a marketing
  // email is ever added, it needs its own unsubscribe link.
  const base = appUrl()
  const addr = LEGAL.mailingAddress || LEGAL.city
  return `<p style="margin:0 0 4px;">Refunds available up to 24h before pickup. Reply or text (636) 266-1801 for help.</p>
      <p style="margin:0 0 4px;">${LEGAL.entity} · ${addr}</p>
      <p style="margin:0;">You are getting this because of a Jville Brew Loop booking. <a href="${base}/terms" style="color:#9c9ca3;">Terms</a> · <a href="${base}/privacy" style="color:#9c9ca3;">Privacy</a> · <a href="${base}/refunds" style="color:#9c9ca3;">Refunds</a></p>`
}
    ...
    ? `<p style="margin:18px 0 14px;color:${INK};font-size:14px;">Show this QR to the driver when you board:</p>
       ${ticketLinks.map(t => {
         ...
         const riderLine = t.name
           ? `<p style="margin:0 0 10px;color:#8a8a92;font-size:12px;letter-spacing:0.04em;text-transform:uppercase;font-weight:700;">${escape(t.name)}</p>`
           : ''
         return `
...
<html><body style="margin:0;padding:0;background:#0a0a0b;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;color:${INK};">
  <div style="max-width:540px;margin:0 auto;padding:24px 20px;">
    ...
    <div style="margin:24px 0 0;padding-top:16px;border-top:1px solid #2a2a31;color:#8a8a92;font-size:12px;line-height:1.5;">
...
// Loop Pass welcome, sent once when the subscription is paid. Same three steps
// as the SMS (sendPassWelcome) and the /pass/success page, so every channel
// tells the member the same thing: the pass makes the seat free, it does not
// book it. They still book each weekend on /book with the phone or email on the pass.
//   member: { firstName }
//   siteUrl: public origin, e.g. https://jvillebrewloop.com
export function passWelcomeHtml({ member, siteUrl }) {
  const greet = member?.firstName ? `You're in, ${escape(member.firstName)}.` : `You're in.`
  const bookUrl = `${siteUrl}/book`
  const manageUrl = `${siteUrl}/pass/manage`
  const step = (n, title, body) => `
    <tr>
      <td valign="top" style="width:36px;padding:0 0 16px;">
        <div style="width:28px;height:28px;border-radius:14px;border:1.5px solid ${GOLD};color:${GOLD};font-size:14px;font-weight:700;text-align:center;line-height:28px;">${n}</div>
      </td>
      <td valign="top" style="padding:3px 0 16px;">
        <p style="margin:0 0 3px;color:${INK};font-size:16px;font-weight:700;">${title}</p>
        <p style="margin:0;color:#9c9ca3;font-size:14px;line-height:1.5;">${body}</p>
      </td>
    </tr>`
...
<html><body style="margin:0;padding:0;background:#0a0a0b;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;color:${INK};">
  <div style="max-width:540px;margin:0 auto;padding:24px 20px;">
    <div style="text-align:center;margin-bottom:18px;">
      <p style="margin:0;color:${GOLD};font-size:11px;letter-spacing:0.22em;text-transform:uppercase;font-weight:700;">Jville Brew Loop · Loop Pass</p>
    </div>

    <h1 style="margin:0 0 10px;color:${INK};font-size:24px;font-weight:700;">${greet}</h1>
    <p style="margin:0 0 22px;color:${INK};font-size:16px;line-height:1.5;">
      Your Loop Pass is active. Every weekend loop is covered for $25 a month. Here's how to ride:
    </p>

    <table role="presentation" cellpadding="0" cellspacing="0" style="width:100%;border-collapse:collapse;">
      ${step(1, 'Book your seat each weekend', `Go to <a href="${escape(bookUrl)}" style="color:${GOLD};">${escape(siteUrl.replace(/^https?:\/\//, ''))}/book</a> and pick your night. Use the same phone number or email you signed up with so we know it's you.`)}
      ${step(2, 'Your seat shows $0', 'Your pass covers it. No payment needed, just confirm the booking.')}
      ${step(3, 'Show your ticket when you board', 'We text and email your ticket. Show it to the driver at pickup.')}
    </table>
    ...
    <div style="padding:14px 16px;border:1px solid #2a2a31;border-radius:10px;color:#9c9ca3;font-size:13px;line-height:1.6;">
      ...
      <p style="margin:0;">Renews monthly. Cancel anytime at <a href="${escape(manageUrl)}" style="color:#9c9ca3;text-decoration:underline;">Manage your Loop Pass</a>.</p>
    </div>

    <div style="margin:24px 0 0;padding-top:16px;border-top:1px solid #2a2a31;color:#6c6c72;font-size:12px;line-height:1.5;">
      <p style="margin:0 0 4px;">Questions? Reply to this email or text (636) 266-1801.</p>
...
export function passWelcomeText({ member, siteUrl }) {
  const greet = member?.firstName ? `You're in, ${member.firstName}.` : `You're in.`
  let body = `${greet}\n\nYour Loop Pass is active. Every weekend loop is covered for $25 a month. Here's how to ride:\n\n`
  body += `1. Book your seat each weekend at ${siteUrl}/book. Use the same phone number or email you signed up with.\n`
  body += `2. Your seat shows $0. No payment needed, just confirm the booking.\n`
  body += `3. We text and email your ticket. Show it to the driver when you board.\n\n`
  body += `Your pass covers one seat: yours. Friends riding with you pay the normal ticket price.\n`
  body += `Book on our site, not Ticket Tailor. That's where your pass applies.\n`
  body += `Renews monthly. Cancel anytime: ${siteUrl}/pass/manage\n\n`
  body += `Questions? Reply to this email or text (636) 266-1801.\nJville Brew Loop`
  return body
}
