...
export async function sendEmail({ to, subject, html, text, replyTo, from }) {
  ...
  // Per-business sender (Surf City passes its own from via SURF_EMAIL_FROM);
  // falls back to the shared EMAIL_FROM, then the Resend onboarding sender.
  const fromAddr = from || process.env.EMAIL_FROM || 'Brew Loop <onboarding@resend.dev>'
  const payload = {
    from: fromAddr,
