...
// buildDefaultSchedule(pickupTime, optsOrFirstStopName)
//
// Two call signatures (back-compat):
//   buildDefaultSchedule('19:30', 'Angry Ginger')        // legacy: only first stop
//   buildDefaultSchedule('19:30', { bars: [...] })       // preferred: real bar names
//
// The legacy form keeps placeholder "Stop 2/3/4/5" labels for callers that
// haven't migrated yet. Prefer { bars: [...] } so the UI never shows generic
// placeholders.
export function buildDefaultSchedule(pickupTime, opts = 'Stop 1') {
  const start = parsePickup(pickupTime)
  if (!start) return null

  let bars = null
  let firstStopName = 'Stop 1'
  if (typeof opts === 'string') {
    firstStopName = opts
  } else if (opts && Array.isArray(opts.bars) && opts.bars.length) {
    bars = opts.bars
  }

  const count = bars?.length || DEFAULT_STOP_COUNT
  const stops = []
  for (let i = 0; i < count; i++) {
    const mins = start + i * DEFAULT_INTERVAL_MIN
    let name
    if (bars) name = bars[i]
    else name = i === 0 ? firstStopName : `Stop ${i + 1}`
    stops.push({ name, start_time: formatMinutes(mins) })
  }
  return stops
}
...
// Friday's Loop stays "tonight" until 9 AM Saturday so the admin can wrap up
// the shift without the page rolling over at midnight. Use this for staff
// surfaces only — rider-facing pages should keep midnight rollover.
export function operationalDateInTZ(
  ...
  if (!Number.isFinite(h) || h >= cutoffHour) return today
