...
import { getStripeMoneyIn, getPaymentSyncHealth } from './stripeMoney'
...
export async function getLeadershipHome() {
  ...
  // Money comes from Stripe, headcounts come from the database. Neither table
  // is a superset of the other — see lib/stripeMoney.js.
  const [live, weekend, money, sync] = await Promise.all([
    getLive(sb),
    getWeekend(sb),
    getStripeMoneyIn({ days: 30 }),
    getPaymentSyncHealth(sb),
  ])
  return { live, weekend, money, sync }
}
