...
async function getActiveGroup(sb) {
  ...
  const { data: groups } = await sb
    ...
    .eq('kind', 'brew')   // leadership "Tonight" strip is Brew Loop only
...
async function getWeek(sb) {
  ...
  // Scope every Brew Loop number to Brew so Marines/Surf don't inflate it.
  // Brew orders are NEVER metadata-tagged with a kind; Marines/Surf orders carry
  // metadata->>kind ('marines'/'surf'), so `metadata->>kind is null` = Brew (+
  // legacy untagged). Brew bars are business='brew'.
  const [orders, sponsorPay, barPay, riderItems, bank, activeBars] = await Promise.all([
    ...
    sb.from('order_items')
      .select('id, orders!inner(status, paid_at, metadata)', { count: 'exact', head: true })
      .is('voided_at', null)
      .eq('orders.status', 'paid')
      .is('orders.metadata->>kind', null)
    ...
    sb.from('bars').select('slug', { count: 'exact', head: true }).eq('status', 'active').eq('business', 'brew'),
  ])
