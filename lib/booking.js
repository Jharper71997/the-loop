...
//
// opts.skipSms — defaults to FALSE. Booking confirmation SMS re-enabled
//   2026-05-05 after Jacob's wife test exposed that the 2026-05-02 SMS
//   disable was suppressing emails too. Independent gates now mean SMS
//   failures (e.g. Twilio toll-free still pending) don't block email.
// opts.skipEmail — defaults to FALSE.
// opts.force — bypass the dedup window (used by self-serve resend).
export async function finalizeBooking(supabase, orderId, opts = {}) {
  const { skipSms = false, skipEmail = false, force = false } = opts
  ...
  if (order.event_id && (!skipSms || !skipEmail)) {
    ...
    if (ev) {
      ...
      if (!skipEmail && order.buyer_email) {
        try {
          const buyerContact = order.contact_id
            ? (await supabase.from('contacts')
                .select('id, first_name, has_signed_waiver, last_confirmation_email_sent_at')
      ...
      const fan = await sendPerRiderConfirmations(supabase, {
        order, event: ev, items, existingByItem, force, skipSms, skipEmail, schedule,
      })
...
// ask the buyer.
async function sendPerRiderConfirmations(supabase, { order, event, items, existingByItem, force, skipSms = false, skipEmail = false, schedule = [] }) {
