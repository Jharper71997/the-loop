...
//
// opts.skipSms — defaults to TRUE. All automatic SMS is disabled per
//   product decision (2026-05-02). Manual admin paths (e.g. self-serve
//   resend button) can pass { skipSms: false } to opt back in.
// opts.skipEmail — defaults to FALSE. Email confirmations stayed on after
//   the 2026-05-02 SMS disable; the gate is independent so toggling SMS
//   doesn't accidentally suppress receipts.
// opts.force — bypass the dedup window (used by self-serve resend).
export async function finalizeBooking(supabase, orderId, opts = {}) {
  const { skipSms = true, skipEmail = false, force = false } = opts
  ...
  if (order.event_id && (!skipSms || !skipEmail)) {
    ...
    if (ev) {
      ...
      const fan = await sendPerRiderConfirmations(supabase, {
        order, event: ev, items, existingByItem, force, skipSms, skipEmail,
      })
...
// ask the buyer.
async function sendPerRiderConfirmations(supabase, { order, event, items, existingByItem, force, skipSms = true, skipEmail = false }) {
