...
export async function finalizeBooking(supabase, orderId, opts = {}) {
  ...
  if (order.event_id && (!skipBuyerSms || !skipBuyerEmail || !skipBuyerPush || !skipRiderSms || !skipRiderEmail || !skipRiderPush)) {
    ...
    // unchanged. isMarines is still used below to skip the (Brew/Surf) waiver.
    const isMarines = ev?.kind === 'marines'
      ...
      if (!skipBuyerEmail && order.buyer_email) {
        try {
          const buyerContact = order.contact_id
            ...
            : null
          const waiverLink = (buyerContact && !isMarines) ? `${appUrl()}/waiver/${buyerContact.id}` : null
...
async function sendPerRiderConfirmations(supabase, { order, event, items, existingByItem, force, skipSms = false, skipEmail = false, skipPush = false, schedule = [] }) {
  ...
  // Per-business pass path + brand. Marines riders get no waiver line (Brew +
  // Surf do — both are bar shuttles).
  ...
  for (const item of items) {
      ...
      try {
        await sendRiderConfirmation(riderPhone, {
          rider: {
            ...
            waiverUrl: isMarines ? null : `${appUrl()}/waiver/${contact.id}`,
            hasSignedWaiver: isMarines ? true : !!contact.has_signed_waiver,
          },
          event,
        })
        await supabase
    ...
    try {
      const waiverLink = isMarines ? null : `${appUrl()}/waiver/${contact.id}`
      const stop = Number.isFinite(item.stop_index) ? schedule[item.stop_index] : null
      const riderTicket = [{
        name: riderName(item),
