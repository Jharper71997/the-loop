...
export async function finalizeBooking(supabase, orderId, opts = {}) {
  ...
  if (order.event_id && (!skipBuyerSms || !skipBuyerEmail || !skipBuyerPush || !skipRiderSms || !skipRiderEmail || !skipRiderPush)) {
    ...
    // The Loop (Marines) is a separate product on the same engine: its pass
    // lives at /marines/tickets/<code> and its confirmations must read as "The
    // Loop", never "Brew Loop" (under-21 riders, no bar/alcohol cues). Brew
    // stays exactly as-is.
    const isMarines = ev?.kind === 'marines'
    const passBase = isMarines ? '/marines/tickets/' : '/tickets/'
    const brand = isMarines ? 'The Loop' : 'Brew Loop'
      ...
      if (!skipBuyerPush && order.contact_id) {
        try {
          await sendPushToContact(order.contact_id, {
            ...
            url: enrichedTicketLinks[0]?.url ? new URL(enrichedTicketLinks[0].url).pathname : (isMarines ? '/marines/my-tickets' : '/my-tickets'),
            tag: `order-${order.id}`,
          })
        } catch (err) {
...
async function sendPerRiderConfirmations(supabase, { order, event, items, existingByItem, force, skipSms = false, skipEmail = false, skipPush = false, schedule = [] }) {
  ...
  // Marines riders get a /marines/tickets/<code> pass and no waiver line.
