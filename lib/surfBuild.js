...
// The Loop (Marines) is a FLAT two-tier day-pass product, NOT per-stop pricing
// like Surf/Brew. Every Marines loop offers exactly these two fares regardless of
// how many stops the route has. Both are "walk-on" (stop_index null) so the rider
// picks their boarding stop (the base gate) from the schedule at checkout, and
// neither is capped — the shuttle's ~13 seats are driver-managed for the trial.
const MARINES_FARES = [
  { name: 'Single Ride', price_cents: 1000 },
  { name: 'All-Day Pass', price_cents: 2000 },
]

// Turn builder/PLAN stops into a groups.schedule array (inline coords win) and
// the ticket_types rows. Returns { schedule, fares }. `kind` selects the fare
// model: 'surf'/'brew' get one per-stop fare each; 'marines' gets the two flat
// MARINES_FARES above, independent of the stop list.
function shapeLoop(loop, kind = 'surf') {
  ...
  const schedule = stops.map(s => {
    ...
    // Preserve the on-base pickup marker so downstream (dispatch, door list) can
    // tell the base gate apart from the town destinations.
    if (s.on_base) entry.on_base = true
    return entry
  })

  if (kind === 'marines') {
    const fares = MARINES_FARES.map((f, i) => ({
      name: f.name,
      price_cents: f.price_cents,
      stop_index: null,   // walk-on: rider chooses pickup (the gate) from the schedule
      capacity: null,     // no hard seat cap for the trial (driver-managed)
      sort_order: i,
      active: true,
    }))
...
// `kind` defaults to 'surf' so every existing Surf caller is unchanged; the
// Marines console + script pass kind='marines' to build The Loop's weekends.
function previewLoop(loop, kind = 'surf') {
  const { schedule, fares } = shapeLoop(loop, kind)
  const groupRow = {
    name: loop.name,
...
async function insertLoop(sb, loop, kind = 'surf') {
...
async function loopExists(sb, eventDate, name, kind = 'surf') {
  if (!eventDate) return false
  const { data: groups } = await sb.from('groups')
    .select('id').eq('event_date', eventDate).eq('kind', kind).eq('name', name)
