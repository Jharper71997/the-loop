...
export async function upsertContactByPhoneOrEmail(supabase, info) {
  ...
  // '*' is a PostgREST wildcard with no escape; such a value is never a real email.
  if (!existing && email && !email.includes('*')) {
    // ilike for case-insensitive match — see lib/ticketTailor.js for context.
    // Escape the LIKE wildcards: '_' is common in emails and would otherwise
    // match any character, attaching a new rider to someone else's contact.
    const { data } = await supabase
      .from('contacts')
      .select('id, has_signed_waiver, waiver_version')
