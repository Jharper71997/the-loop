...
const ACTIVE_BARS_TARGET = 5
const RIDERS_WEEKEND_TARGET = 30
const DRIVERS_TARGET = 2
...
export async function getScoreboard() {
  ...
  const [
    ...
    supabase.from('order_items')
      .select('id', { count: 'exact', head: true })
      .gte('checked_in_at', weekend.start.toISOString())
      .lt('checked_in_at', weekend.end.toISOString()),
  ...
  return {
    ...
    netProfitMTD: {
      ...
      drillTo: '/leadership/profit-first',
    ...
    weekendRiders: {
      ...
      value: weekendOrderItems?.count ?? 0,
      target: RIDERS_WEEKEND_TARGET,
      status: status(weekendOrderItems?.count ?? 0, RIDERS_WEEKEND_TARGET),
      drillTo: '/leadership/income',
      note: (weekendOrderItems?.count ?? 0) === 0 ? 'No check-ins logged for this past Fri/Sat' : null,
    },
    drivers: {
      label: 'Active Drivers',
      value: activeDrivers?.count ?? 0,
      target: DRIVERS_TARGET,
      status: status(activeDrivers?.count ?? 0, DRIVERS_TARGET),
      drillTo: null,
      note: null,
    },
  }
