...
import { SITE, isLoopSite } from './site'
...
export const BUSINESS = {
  brew: {
    ...
    // Rider-facing support line shown in "text us" copy. Marines/Surf share the
    // Brew staffed line today; give them their own here if they get a number.
    contactPhone: '+16362661801',
    contactPhoneDisplay: '(636) 266-1801',
    adminCodeEnv: null,
    driverCodeEnv: null,
  },
  marines: {
    ...
    barsBusiness: 'marines', // partner-bar directory, same model as Brew/Surf
    emailFromEnv: 'EMAIL_FROM',
    smsPhoneEnv: 'SIMPLETEXTING_PHONE',
    contactPhone: '+16362661801',        // shares the Brew staffed line for now
    contactPhoneDisplay: '(636) 266-1801',
...
// On a standalone single-business deployment (see lib/site.js) that business
// owns the root, so its rider links lose the prefix: /marines/track is just
// /track. Precomputed once rather than branching at every call site.
const ROOTED = Object.fromEntries(
  Object.entries(BUSINESS).map(([kind, cfg]) => [kind, {
    ...cfg,
    basePath: '',
    ticketPath: '/tickets/',
    myTicketsPath: '/my-tickets',
...
export function brandFor(kind) {
  const cfg = BUSINESS[kind] || BUSINESS.brew
  // Only the business this deployment IS gets rooted. Everything else keeps
  // its prefixed paths, which matters on the combined Brew host where all
  // three coexist and on any cross-business link.
  if (isLoopSite && cfg.kind === SITE) return ROOTED[cfg.kind]
  return cfg
}
...
// Client-safe: which business a rider pathname belongs to. Surf and Marines are
// the prefixed rider sites (/surfcity, /marines) on the shared (external) chrome;
// anything else is Brew (root).
//
// The prefixes are still checked first on a standalone site because middleware
// rewrites the public URL onto the /marines tree — a component reading the
// server-rendered path can legitimately see either spelling.
export function businessFromPath(pathname) {
  const p = String(pathname || '')
  if (p.startsWith('/surfcity')) return 'surf'
  if (p.startsWith('/marines')) return 'marines'
  // On a standalone single-business site the root belongs to that business,
  // not to Brew.
  if (isLoopSite) return SITE
  return 'brew'
}
