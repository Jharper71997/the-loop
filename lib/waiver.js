...
// Lowest waiver version a signer can be on without needing to re-sign. The
// floor only moves when an admin marks a new version with requires_resign=true
// (i.e. the legal text materially changed). Typo fixes and formatting tweaks
// stay below the floor so prior signers stay valid.
export async function getMinimumAcceptableVersion(supabase) {
  const sb = supabase || supabaseAdmin()
  const { data } = await sb
    .from('waiver_versions')
    .select('version')
    .eq('requires_resign', true)
    .order('version', { ascending: false })
    .limit(1)
    .maybeSingle()
...
// Returns true if the contact has a signature for any waiver version at or
// above the minimum acceptable floor. Lets admins fix typos without
// invalidating everyone who signed the prior version.
export async function contactHasSignedCurrent(supabase, contactId) {
  if (!contactId) return false
  const sb = supabase || supabaseAdmin()
  const minVersion = await getMinimumAcceptableVersion(sb)

  const { data: sigs } = await sb
    .from('waiver_signatures')
    .select('waiver_versions ( version )')
    .eq('contact_id', contactId)
  if (!sigs?.length) return false

  for (const s of sigs) {
    const v = s.waiver_versions?.version
    if (Number.isFinite(v) && v >= minVersion) return true
  }
  return false
}
...
export async function textUnsignedContact(supabase, contact, { force = false } = {}) {
  ...
  // Every other send path checks this (booking.js, ticket-reminder,
  // ride-feedback); this one didn't, so it texted riders who had explicitly
  // opted out. `force` skips the dedupe window, never consent.
  if (contact.sms_consent === false) return { skipped: 'no_consent' }
