...
export async function sendBookingConfirmation(to, { buyer, event, ticketLinks = [] }) {
  ...
  // The Loop (Marines) rides on the same engine but must never read "Brew Loop".
  const marines = event?.kind === 'marines'
  const brand = marines ? 'The Loop' : 'Brew Loop'
  ...
  body += `\n\nLost it? ${appUrl()}${marines ? '/marines/my-tickets' : '/my-tickets'}`
...
export async function sendRiderConfirmation(to, { rider, event }) {
  ...
  const ride = event?.kind === 'marines' ? 'The Loop' : 'the Brew Loop'
