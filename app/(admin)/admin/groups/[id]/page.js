...
export default async function ManageLoopPage({ params }) {
  ...
  return (
    <main style={{ maxWidth: 1000, margin: '0 auto', padding: '20px 16px', minHeight: '100vh', color: '#fff' }}>
      <a href="/admin/groups" style={{ color: ACCENT, fontSize: 13, textDecoration: 'none' }}>← All Loops</a>
      <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginTop: 8, gap: 10, flexWrap: 'wrap' }}>
        <h1 style={{ fontSize: 'clamp(22px, 5vw, 26px)', margin: 0 }}>{group.name}</h1>
        {event && (
