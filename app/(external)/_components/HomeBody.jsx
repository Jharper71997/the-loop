...
export default function HomeBody({ loops = [], business = 'brew' }) {
  ...
  return (
    <main style={{ padding: '14px 14px 24px' }}>
      <div style={{ maxWidth: 520, margin: '0 auto', display: 'grid', gap: 12 }}>
        <NextLoopCard loop={next} business={business} />

        <QuickChips business={business} />

        {more.length > 0 && (
          <section style={{ marginTop: 4 }}>
            <div style={sectionLabel}>Upcoming</div>
            <div style={{ display: 'grid', gap: 8, marginTop: 6 }}>
              {more.map(loop => <LoopRow key={`${loop.kind}-${loop.id}`} loop={loop} business={business} />)}
            </div>
          </section>
        )}
      </div>
...
function NextLoopCard({ loop, business }) {
  ...
  if (!loop) {
    return (
      <section style={{ ...cardBase, padding: '28px 22px', display: 'grid', justifyItems: 'center', textAlign: 'center', gap: 12 }}>
        ...
        <a href={prefixLink('/events', business)} style={ghostCta}>See full calendar</a>
      </section>
    )
  }

  const href = prefixLink(`/book/${loop.id}`, business)
    ...
    >
      ...
      <div style={{ position: 'relative', padding: '18px 18px 20px', display: 'grid', gap: 14 }}>
        ...
        <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <a href={prefixLink('/my-tickets', business)} style={ghostCtaInline}>I have a ticket</a>
        </div>
      </div>
    </section>
  )
...
function LoopRow({ loop, business }) {
  const cfg = brandFor(business)
  const href = prefixLink(`/book/${loop.id}`, business)
  return (
    <a
