...
export default async function LeadershipScoreboard() {
  ...
  const sections = [
    ...
    {
      title: 'Growth',
      subtitle: 'Is the business growing?',
      cards: [
        { ...data.mrr,
          value: formatCents(data.mrr.valueCents),
          target: formatCents(data.mrr.targetCents) + ' / mo',
        },
        { ...data.activeSponsors,
        ...
        { ...data.momGrowth,
          value: data.momGrowth.value != null
            ? (data.momGrowth.value >= 0 ? '+' : '') + formatPercent(data.momGrowth.value)
            : '—',
          target: '≥ ' + formatPercent(data.momGrowth.target),
        },
      ],
    },
  ]
