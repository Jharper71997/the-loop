  ...
  const columns = [
    {
      ...
      render: e => (
        <Link href={`/leadership/drivers/route-log/${e.id}`} style={{ fontWeight: 600, color: '#17130f', textDecoration: 'none' }}>
          {e.name || 'Untitled Loop'}
        </Link>
      ),
    },
    { key: 'date', header: 'Date', render: e => <span style={{ color: '#6e6154', fontSize: 13 }}>{e.event_date ? formatLoopDate(e.event_date) : '—'}</span> },
    ...
    },
    {
      ...
      render: e => <Link href={`/leadership/drivers/route-log/${e.id}`} style={{ color: '#8a5f0a', fontSize: 13, textDecoration: 'none', fontWeight: 500 }}>view →</Link>,
    },
  ]
    ...
    <main style={pageStyle}>
      <div style={containerStyle}>
        ...
        <DataTable
          ...
          empty={(
            <div style={emptyBox}>
              No route logs yet. Open a Loop in <Link href="/leadership/loops" style={{ color: '#8a5f0a' }}>Loops</Link> and click <strong>Generate route log</strong> to seed the 25 stops.
            </div>
          )}
        />
      </div>
...
function ProgressBar({ pct }) {
  return (
    <div style={{ width: 120, maxWidth: '100%', height: 7, background: '#f3ecdd', borderRadius: 999, overflow: 'hidden', border: '1px solid #e8ddc8' }}>
      <div style={{ width: `${pct}%`, height: '100%', background: pct >= 100 ? '#2fa36b' : '#d4a333' }} />
    </div>
  )
}
...
const emptyBox = {
  background: '#ffffff', border: '1px solid #e8ddc8', borderRadius: 8,
  padding: '20px 22px', color: '#6e6154', fontSize: 14,
}
