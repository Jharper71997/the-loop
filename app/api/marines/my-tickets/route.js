...
// POST /api/marines/my-tickets
//   body: { phone }
// Returns the most recent paid + pending orders for any rider whose phone
// matches (buyer_phone OR any order_items.rider_phone), SCOPED TO The Loop
// (Marines) events only — events.kind = 'marines'. A red-line fork of
// /api/my-tickets with the Brew Loop bits stripped: no waiver, no referral,
// no security chat. Boarding-pass links point at /marines/tickets/<code>.
//
// Privacy: the response only includes first names, no last names, no emails.
// Throttled per IP and per submitted phone via the Postgres limiter (sql/054).
// The in-memory counter this replaced was per lambda instance and reset on
// every cold start, so it never really capped anything.
...
export async function POST(req) {
  ...
  if (!(await rateLimit('marines_my_tickets_ip', clientIp(req), 20, 60))) {
  ...
  // Key difference from Brew: scope to The Loop (Marines) events only. Drop any
  // order whose event isn't a marines event so Brew passes never surface here.
  const all = [...(byBuyer || []), ...extraOrders]
    .filter(o => o.event?.kind === 'marines')
    .sort((a, b) => (b.created_at || '').localeCompare(a.created_at || ''))

  // One per-rider QR-code lookup so we can deep-link each rider to their
  // boarding pass at /marines/tickets/<code>. Pending orders won't have any
