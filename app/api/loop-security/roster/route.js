...
// GET /api/loop-security/roster
// Door list for the active Marines loop: every paid, non-voided rider with
// their pass type + check-in status + whether they're currently on board.
// Code-gated (loop_admin cookie). No event id in the URL — it resolves the
// active Marines loop itself. The Loop has no Ticket Tailor + no waivers.
export async function GET() {
  if (!(await isLoopAdmin())) {
    return Response.json({ error: 'forbidden' }, { status: 403 })
  }

  const loop = await getActiveMarinesLoop().catch(() => null)
