...
// Surf City Loop route builder API, re-homed into the /admin app from the old
// code-gated /api/surf-admin/loops. Purpose-built for the multi-loop, stop-first
// model — the Surf analog of /api/events but WITHOUT its one-event-per-group
// guard or schedule-derived-from-ticket-types behavior (Brew's /api/events stays
// frozen). A Surf "loop" = one group + one event + one ticket_type PER STOP, with
// stops stored inline (name + start_time + lat/lng) in groups.schedule. A weekend
...
// PUT ?group_id= — edit one loop in place. Updates the group (name/date/time +
