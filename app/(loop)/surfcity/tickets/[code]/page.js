...
// Surf City Loop boarding pass. A fork of marines/tickets/[code] reading the
// same qr_codes(kind='checkin') -> order_item chain directly from Supabase
// (no Marines-only API), so the SAME code the door scanner reads is what shows
// here. Scoped to kind='surf' events — a Brew or Marines code 404s here.
export default async function SurfTicketPage({ params }) {
  ...
  if (event?.kind !== 'surf') notFound()

  // Unclaimed claim-link seat — bounce to the claim flow (same as Brew/Marines).
  ...
  // First stop on the route is the first pickup.
  ...
  const ticketUrl = `${appUrl()}${cfg.ticketPath}${code}`
