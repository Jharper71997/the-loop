...
const GREEN = '#5fc97a'
const BRAND = 'Surf City Loop'
const BACK_HREF = '/surfcity'

// Surf City Loop boarding pass — gold-themed fork of the Marines TicketView. No
// waiver, no security chat, no ID gate. Keeps the screen awake + share so the
// rider can flash the QR to the driver when they board.
export default function LoopTicketView({
  ...
  async function onShare() {
    const text = `${riderName} — ${BRAND} pass\n${ticketUrl}`
    if (navigator.share) {
      try { await navigator.share({ title: `${BRAND} pass`, text, url: ticketUrl }) } catch {}
    ...
    return (
      <div className="external-shell" style={{ minHeight: '100dvh', background: C.BG, color: C.INK, padding: '24px 16px 48px',
        ...
        <div style={{ maxWidth: 420, width: '100%', textAlign: 'center', display: 'grid', gap: 14 }}>
          ...
          <a href={BACK_HREF} style={primaryBtn}>Back to {BRAND}</a>
        </div>
      </div>
    )
  }
      ...
      <div style={{ maxWidth: 460, width: '100%', display: 'grid', gap: 18 }}>
        ...
        {(pickupSpot || timeLabel) && (
          <div style={{ background: 'rgba(212,163,51,0.08)', border: `1px solid rgba(212,163,51,0.35)`,
            ...
            <div style={{ color: C.INK, fontSize: 18, fontWeight: 700 }}>{pickupSpot || 'First stop'}</div>
            {timeLabel && (
        ...
        <p style={{ color: C.INK_DIM, fontSize: 12, textAlign: 'center', margin: '4px 0 0' }}>
          Show this screen to the driver when you board.
        </p>
      </div>
    </div>
  )
