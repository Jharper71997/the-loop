...
// Code gate for the Surf City Loop driver surface (/api/surf-driver →
// SURF_DRIVER_CODE cookie). Mirrors SurfAdminGate.
export default function SurfDriverGate() {
  ...
  async function submit(e) {
    ...
    try {
      const res = await fetch('/api/surf-driver', {
  ...
  return (
    <main style={{ padding: '40px 16px', display: 'grid', placeItems: 'center', minHeight: '70dvh' }}>
      <form onSubmit={submit} style={{ width: '100%', maxWidth: 340,
        ...
        <button type="submit" disabled={submitting} style={{ padding: '12px 18px', borderRadius: 10, border: 'none', background: `linear-gradient(180deg, ${GOLD_HI}, ${GOLD})`, color: '#0a0a0b', fontWeight: 800, fontSize: 14, letterSpacing: '0.08em', textTransform: 'uppercase', cursor: submitting ? 'default' : 'pointer', opacity: submitting ? 0.6 : 1 }}>
