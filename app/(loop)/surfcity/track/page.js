// Surf City Loop — live tracking. Mirrors marines/track but loads the active
// kind='surf' loop: reads its inline-coord stops and renders the live map
// (LoopTrackMap polls /api/shuttle/current scoped to this group_id, so it never
// shows the Brew Loop or Marines bus). Surf runs multiple loops per day; this
// shows the active/primary one from getActiveSurfLoop().
...
export const metadata = {
  title: 'Live',
  description: 'See the Surf City Loop shuttle live — where it is on the route and which stop is next.',
  alternates: { canonical: cfg.trackPath },
}
export const dynamic = 'force-dynamic'

// Surf City / Topsail Island area — map center when no stops have coords yet.
const FALLBACK_CENTER = { lat: 34.4274, lng: -77.5466 }
      ...
      <div style={{ maxWidth: 720, margin: '0 auto', display: 'grid', gap: 14 }}>
        <header style={{ padding: '4px 4px 0' }}>
          ...
          <h1 style={{ color: C.INK, fontSize: 26, fontWeight: 800, margin: '6px 0 0', lineHeight: 1.1, letterSpacing: '-0.015em' }}>
            {loop?.name || `${cfg.shortBrand} shuttle`}
          </h1>
          {loop && (
        ...
        <a href={cfg.basePath} style={{ ...ghostCta, display: 'block', textAlign: 'center' }}>Back to {cfg.shortBrand}</a>
      </div>
    </main>
  )
}
