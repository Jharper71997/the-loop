...
// Phone-lookup for Surf City Loop passes — gold-themed fork of Brew Loop's
// MyTicketsClient. POSTs the phone to /api/surf/my-tickets, then lists each
// order with its event date and every rider's pass (name, type chip, QR
// thumbnail, and a link to the full pass at /surfcity/tickets/<code>). No waiver
// pills, no bar/alcohol cues.
export default function SurfMyTicketsClient() {
  ...
  async function lookup(rawPhone) {
    ...
    setError(null)
    try {
      const res = await fetch('/api/surf/my-tickets', {
        method: 'POST',
